...
export async function POST(req: NextRequest) {
  const { transactions, thisMonthSpend, lastMonthSpend, categories, upcomingBills } = await req.json();
...
Write the narrative now. Be conversational, insightful, and actionable. End with one specific saving tip. No markdown headers, just flowing paragraphs.`;
